...
import { fetchJson, normalizeLogs, normalizeBudgets, sumByKind, yen, ymOf, LogItem, BudgetItem } from '@/lib/finance';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!active) return;
        setLogs(normalizeLogs(rawL).filter(x => ymOf(x.occurredAt) === ym));
  ...
  const pctExp = budExp === 0 ? 0 : Math.min(100, Math.round((usedExp / budExp) * 100));
  const pctInc = budInc === 0 ? 0 : Math.min(100, Math.round((usedInc / budInc) * 100));
    ...
    <main className="mx-auto max-w-5xl px-6 py-6 text-slate-100">
      ...
      <section className="space-y-4">
        <div className="rounded-2xl border border-slate-700 p-4">
          ...
          <div className="mb-2">予算：{yen(budExp)}円 / 使用：{yen(usedExp)}円</div>
          <div className="h-2 bg-slate-700 rounded overflow-hidden">
            <div className="h-full bg-emerald-500" style={{ width: `${pctExp}%` }} />
          </div>
        </div>

        <div className="rounded-2xl border border-slate-700 p-4">
          <div className="mb-2 text-sm text-slate-400">収入（当月）</div>
          <div className="mb-2">目標：{yen(budInc)}円 / 実績：{yen(usedInc)}円</div>
          <div className="h-2 bg-slate-700 rounded overflow-hidden">
            <div className="h-full bg-sky-500" style={{ width: `${pctInc}%` }} />
          </div>
        </div>
